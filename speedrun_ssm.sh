...
# Make sure the cache directory is a symlink to the scratch directory
[ -L "$HOME/.cache" ] || { echo "~/.cache is not a symlink" >&2; exit 1; }
...
# torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.mid_train -- --run=$WANDB_RUN --model-tag=$MODEL_TAG --device_batch_size=8
